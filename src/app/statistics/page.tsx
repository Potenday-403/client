...
import { EVENT_TYPE, EVENT_TYPE_LABEL, EventType } from "@/models/shared";
      ...
      <div className="mt-2 flex items-center justify-between px-6">
        {STEPS.map((step, index) => (
          <p
            ...
            className={cn(
              "text-sm",
              currentStep > index ? "text-accents-5" : "text-accents-2",
            )}
          >
            {step}
          </p>
      ...
      <Card className="mt-5">
        <Card.Content>
          ...
          <p className="mt-3 text-[20px] font-semibold">
            총 {statistics.total.toLocaleString()}
          </p>
...
const Stepper = ({ currentStep, totalSteps, className }: StepperProps) => {
  ...
  return (
    <div
      className={cn(
        "h-1 w-full overflow-hidden rounded-full bg-accents-1",
        className,
      )}
    >
      <div
